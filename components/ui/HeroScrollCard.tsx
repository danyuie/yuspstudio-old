...
const HeroScrollCard = ({title, desc, linkImage,styleImage,style}:HeroScrollCardProps) => {
  return (
   <div className='mr-2 md:mr-4'>
     <div className={`rounded-xl overflow-hidden bg-white w-full dark:bg-neutral-800/30 ${style?style:''}`}>
      ...
      <div className='h-1/2 p-4 flex flex-col gap-2'>
        <div className='font-bold text-base tracking-tight text-slate-950 dark:text-slate-50'>{title}</div>
        <div className='text-xs text-gray-400 line-clamp-2 dark:text-neutral-400'>{desc}</div>
      </div>
    </div>
   </div>
  )
